...
| 1. Financial correctness and Mesh slice | Complete | Checked fixed-point math, cross-boundary parsers, native Solana read packages, runtime telemetry, tests, and a compiled collector are pinned to Mesh `21a8736` |
...
| 8. Shadow and P1 Mesh expansion | In progress | Strict simulation-only Jupiter/perp action fixtures pass the independent Rust policy dry-run, account/fee/compute deltas are durably compared with paper estimates, unknown results block retry until reconciliation, and the first compiled read-only native Mesh Solana proof validates JitoSOL state against the authoritative adapter; exact current transaction/order construction, RPC/venue simulation, sustained feed differential soak, and authoritative calibration remain gated on a qualified shadow identity |
