...
| 1. Financial correctness and Mesh slice | Complete | Checked fixed-point math, cross-boundary parsers, native Solana read packages, runtime telemetry, tests, and a compiled collector are pinned to Mesh `c5c75c4` |
...
| 8. Shadow | In progress | Strict simulation-only Jupiter/perp action fixtures pass the independent Rust policy dry-run, account/fee/compute deltas are durably compared with paper estimates, and unknown results block retry until reconciliation; exact current transaction/order construction, RPC/venue simulation, and authoritative calibration remain gated on a qualified shadow identity |
