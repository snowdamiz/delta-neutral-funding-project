...
|---|---|---|
| `solana_wallet_flow_quant` (centerpiece) | v2 implemented, realtime capture; frozen 90-day validation not started | Schema 50 replaces the v1 broker's amputated exit path (15-minute hard stop, no profit-taking, exit-on-migration, exit-on-one-bad-print) with a recoup-then-ride engine: $100 positions, up to 3 slots, take-profit ladder that recoups cost at 2×, trailing stop on the executable quote's high-water mark, hard stop-loss, flat-only time stop, debounced flow-collapse and no-liquidity exits, and hold-through-migration with the crossing recorded. Schema 51 adds wallet discovery: every candidate snapshot persists its earliest unlinked buyers, and wallets repeatedly early into mints that later ran are nominated in the read model — promotion is the existing audited cohort mutation. Schema 52 adds default-off live execution (below). Acquisition capture is realtime: one `logsSubscribe` per followed wallet over the provider socket, reconciled against the cohort so wallets are added without provider-side configuration or a restart, with a slot heartbeat, reconnect-then-sweep, and the unchanged durable-cursor path behind every notification. `db/tests/solana_paper_broker.sql`, `wallet_discovery.sql`, `solana_live_mode.sql`, adapter and UI tests are executable evidence |
