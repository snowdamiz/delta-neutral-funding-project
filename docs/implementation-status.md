...
| 2. Production-like Mesh foundation | In progress | Local Docker services, fenced renewable writer lease with graceful release, accepted-request drain, database-authoritative startup reconciliation, pre-allocation HTTP request limits, bounded-delivery overload/concurrency and GC probes, health, metrics, structured logs, database pooling, memory/PID caps, restart policies, and bounded local logs are present; long elapsed soak remains |
