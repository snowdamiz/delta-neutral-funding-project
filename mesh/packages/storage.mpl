...
pub fn persist_opportunities(pool :: PoolHandle, body :: String, snapshot :: MarketSnapshot, result :: OpportunitySet, config_hash :: String) -> Int ! String do
  let rows = Pool.query(pool, "WITH source_event AS (INSERT INTO normalized_events (id, schema_version, event_type, source, observed_at_ms, source_sequence, idempotency_key, raw_payload_hash, canonical_payload) VALUES ($1, 1, 'MarketSnapshot', $2, $3::bigint, $4, $5, $6, $7::jsonb) ON CONFLICT (idempotency_key) DO NOTHING RETURNING id), decisions AS (INSERT INTO opportunity_decisions (id, source_event_id, variant, observed_at_ms, nav_lamports, hedge_lamports, expected_funding_usd_micros, nav_reward_usd_micros, net_carry_usd_micros, eligible, reason_code, config_hash) SELECT $1 || ':sol', id, 'sol_control'::strategy_variant, $3::bigint, $8, $9, $10, '0', $11, $12::boolean, $13, $18 FROM source_event UNION ALL SELECT $1 || ':jitosol', id, 'jitosol_carry'::strategy_variant, $3::bigint, $8, $9, $10, $14, $15, $16::boolean, $17, $18 FROM source_event RETURNING id) SELECT count(*)::text AS inserted FROM decisions", [
    snapshot.event_id,
    snapshot.source,
