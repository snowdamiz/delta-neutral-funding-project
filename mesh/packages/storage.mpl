...
plan :: PositionPlan) -> Int ! String do
  ...
  let rows = Pool.query(pool, "SELECT CASE WHEN $9::boolean THEN apply_paper_recovery_plan($1, $2::bigint, $3, $4::jsonb, $5::jsonb, $6, $7::jsonb, $8) ELSE apply_paper_position_plan($1, $2::bigint, $3, $4::jsonb, $5::jsonb, $6, $7::jsonb, $8) END::text AS applied", [
    portfolio_id,
...
pub fn persist_funding_settlement(
  ...
  body :: String,
  sol_payment :: String,
  jitosol_payment :: String
) -> FundingPersistence ! String do
  let rows = Pool.query(pool, "WITH applied AS (SELECT apply_funding_settlement($1::jsonb, $2::jsonb, $3::jsonb) AS result) SELECT result->>'insertedEvent' AS inserted_event, result->>'payments' AS payments FROM applied", [
    body,
    sol_payment,
    jitosol_payment
  ]) ?
