...
pub fn load_paper_runtime(
  ...
  let rows = Pool.query(pool, "SELECT p.state::text, p.state_version::text, p.random_state::text, (c.pause_entries OR c.pause_all OR NOT strategy_enabled(p.variant::text))::text AS paused, c.pause_all::text FROM portfolio_runs p CROSS JOIN control_state c WHERE p.id = $1", [portfolio_id]) ?
  if List.length(rows) != 1 do
