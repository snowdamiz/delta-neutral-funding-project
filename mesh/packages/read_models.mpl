...
pub fn strategies(pool :: PoolHandle) -> String ! String do
  read_body(pool, "WITH catalog(ordinal, id, display_name, family, legs, benchmark_id, mode) AS (VALUES (1, 'sol_control', 'SOL control', 'carry', jsonb_build_array('long SOL spot', 'short SOL-PERP'), '', 'paper'), (2, 'jitosol_carry', 'JitoSOL carry', 'carry', jsonb_build_array('long JitoSOL spot', 'short SOL-PERP at protocol NAV'), 'sol_control', 'paper'), (3, 'cross_asset_funding', 'Cross-asset funding', 'carry', jsonb_build_array('long top-ranked spot asset', 'short matching perpetual'), 'sol_control', 'paper'), (4, 'negative_funding_reverse', 'Negative-funding reverse carry', 'carry', jsonb_build_array('borrow and sell ranked spot asset', 'long matching perpetual'), 'sol_control', 'paper'), (5, 'jitosol_nav_discount', 'JitoSOL NAV discount', 'arbitrage', jsonb_build_array('buy JitoSOL below protocol NAV', 'short NAV-equivalent SOL-PERP', 'redeem through the best modeled exit'), 'sol_control', 'paper'), (6, 'cross_venue_funding', 'Cross-venue funding arbitrage', 'arbitrage', jsonb_build_array('short the high-funding perpetual', 'long the low-funding perpetual'), 'sol_control', 'paper'), (7, 'hyperliquid_wallet_flow', 'Hyperliquid wallet flow', 'signal', jsonb_build_array('aggregate qualified wallet direction', 'filter Phase 1-4 entries'), 'cross_asset_funding', 'paper'), (8, 'hyperliquid_wallet_mirror', 'Hyperliquid wallet mirror', 'signal', jsonb_build_array('mirror qualified wallet direction', 'size from local risk limits'), 'cross_asset_funding', 'paper'), (9, 'hyperliquid_wallet_fade', 'Hyperliquid wallet fade', 'signal', jsonb_build_array('fade consistently losing wallets', 'size from local risk limits'), 'cross_asset_funding', 'paper'), (10, 'solana_wallet_flow_quant', 'Solana wallet-flow quant', 'signal', jsonb_build_array('follow configured wallet acquisitions', 'survival-score the exact mint', 'paper-enter only after executable checks'), '', 'paper')), runs AS (SELECT variant::text AS id, jsonb_agg(id ORDER BY comparison_group_id, id) AS portfolio_run_ids, count(*) FILTER (WHERE state NOT IN ('idle', 'paused')) AS working, count(*) FILTER (WHERE state = 'paused') AS paused, count(*) AS total FROM portfolio_runs WHERE strategy_run_id = 'local-paper-run' GROUP BY variant), items AS (SELECT jsonb_build_object('id', c.id, 'displayName', c.display_name, 'family', c.family, 'legs', c.legs, 'benchmarkStrategyId', NULLIF(c.benchmark_id, ''), 'mode', c.mode, 'controlScope', 'strategy', 'enabled', control.enabled, 'runState', CASE WHEN c.id = 'solana_wallet_flow_quant' AND EXISTS (SELECT 1 FROM solana_paper_positions WHERE status = 'open') THEN 'active' WHEN COALESCE(r.working, 0) > 0 THEN 'active' WHEN NOT control.enabled THEN 'paused' WHEN c.id = 'solana_wallet_flow_quant' THEN 'idle' WHEN r.total IS NULL THEN 'unregistered' WHEN r.paused = r.total THEN 'paused' ELSE 'idle' END, 'portfolioRunIds', COALESCE(r.portfolio_run_ids, '[]'::jsonb)) AS item FROM catalog c JOIN strategy_controls control ON control.strategy_id = c.id LEFT JOIN runs r ON r.id = c.id ORDER BY c.ordinal) SELECT jsonb_build_object('schemaVersion', 2, 'strategies', COALESCE(jsonb_agg(item), '[]'::jsonb))::text AS body FROM items", [])
end
