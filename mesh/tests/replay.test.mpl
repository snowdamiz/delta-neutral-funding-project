...
fn bundle(config :: String, events :: List<String>) -> String do
  let manifest = "{\"replaySchemaVersion\":1,\"bundleId\":\"calm-v1\",\"configHash\":\"${Crypto.sha256(config)}\",\"meshCommit\":\"21a8736c8e01453a2fe8025f00b72a8fd5141050\"}"
...
describe("deterministic replay") do
  test("uses virtual event time and rejects look-ahead ordering") do
    ...
    case run_replay(config, bundle(config, events), "21a8736c8e01453a2fe8025f00b72a8fd5141050") do
      Ok(first) -> do
        case run_replay(config, bundle(config, events), "21a8736c8e01453a2fe8025f00b72a8fd5141050") do
          Ok(second) -> do
