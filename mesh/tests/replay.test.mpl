...
fn bundle(config :: String, events :: List<String>) -> String do
  let manifest = "{\"replaySchemaVersion\":1,\"bundleId\":\"calm-v1\",\"configHash\":\"${Crypto.sha256(config)}\",\"meshCommit\":\"ac039696c3c60e2fba15e45184590212cb785c64\"}"
...
describe("deterministic replay") do
  test("uses virtual event time and rejects look-ahead ordering") do
    ...
    case run_replay(config, bundle(config, events), "ac039696c3c60e2fba15e45184590212cb785c64") do
      Ok(first) -> do
        case run_replay(config, bundle(config, events), "ac039696c3c60e2fba15e45184590212cb785c64") do
          Ok(second) -> do
