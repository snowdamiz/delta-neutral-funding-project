...
fn bundle(config :: String, events :: List<String>) -> String do
  let manifest = "{\"replaySchemaVersion\":1,\"bundleId\":\"calm-v1\",\"configHash\":\"${Crypto.sha256(config)}\",\"meshCommit\":\"e612743eb9b2dd045c0cb50304f88d262e5356d7\"}"
...
describe("deterministic replay") do
  test("uses virtual event time and rejects look-ahead ordering") do
    ...
    case run_replay(config, bundle(config, events), "e612743eb9b2dd045c0cb50304f88d262e5356d7") do
      Ok(first) -> do
        case run_replay(config, bundle(config, events), "e612743eb9b2dd045c0cb50304f88d262e5356d7") do
          Ok(second) -> do
