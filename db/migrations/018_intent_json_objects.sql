...
CREATE OR REPLACE FUNCTION apply_synchronized_paper_entries(
...
BEGIN
  ...
  v_sol_applied := apply_paper_plan(
    ...
    p_source_event_id,
    p_sol_entry->'plan',
    (p_sol_entry->>'spotIntent')::jsonb,
    (p_sol_entry->>'spotIntentHash')::char(64),
    (p_sol_entry->>'perpIntent')::jsonb,
  ...
  v_jito_applied := apply_paper_plan(
    ...
    p_source_event_id,
    p_jito_entry->'plan',
    (p_jito_entry->>'spotIntent')::jsonb,
    (p_jito_entry->>'spotIntentHash')::char(64),
    (p_jito_entry->>'perpIntent')::jsonb,
...
CREATE OR REPLACE FUNCTION apply_synchronized_paper_position_plans(
  p_comparison_group_id text,
  p_source_event_id text,
  p_sol_record jsonb,
  p_jito_record jsonb
...
BEGIN
  ...
  v_sol_applied := apply_paper_position_plan(
    ...
    p_source_event_id,
    p_sol_record->'plan',
    (p_sol_record->>'spotIntent')::jsonb,
    (p_sol_record->>'spotIntentHash')::char(64),
    (p_sol_record->>'perpIntent')::jsonb,
  ...
  v_jito_applied := apply_paper_position_plan(
    ...
    p_source_event_id,
    p_jito_record->'plan',
    (p_jito_record->>'spotIntent')::jsonb,
    (p_jito_record->>'spotIntentHash')::char(64),
    (p_jito_record->>'perpIntent')::jsonb,
...
WHERE jsonb_typeof(intent_json) = 'string'
  AND jsonb_typeof((intent_json #>> '{}')::jsonb) = 'object';

ALTER TABLE execution_intents
  ADD CONSTRAINT execution_intents_json_object CHECK (
